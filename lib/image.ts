...
export function emRadianos(anguloEmGraus: number): number {
...
class Retangulo extends RetLike {
  desenha(x: number | null = null, y: number | null = null) {
    ...
    ctx!.save()
    ctx!.translate(x, y)
    ctx!.rotate(emRadianos(this.angulo))
...
class Elipse extends RetLike {
  desenha(x: number | null = null, y: number | null = null) {
    ...
    ctx!.ellipse(x, y, this.largura / 2, this.altura / 2, emRadianos(this.angulo), 0, 2 * Math.PI)
    if (this.modo === ModoImagem.OUTLINE) {
      ctx!.strokeStyle = this.cor
      ctx!.stroke()
    } else {
...
class ImagemDeArquivo extends Imagem {
  ...
  private desenhaRotacionando(x: number, y: number) {
    let ctx = canvas.getContext('2d')
    ctx!.save()
    ctx!.translate(x, y)
    ctx!.rotate(emRadianos(this.angulo))
...
class TextoImagem extends Imagem {
  ...
  constructor(texto: string, fonte: string, tamanho: string, cor: string) {
    super(0, 0, 0)
    this.texto = texto
    this.fonte = fonte
 ...
 *
 * @param texto
 * @param fonte Ver propriedade 'font' do HTML Canvas: https://www.w3schools.com/jsref/canvas_font.asp
 * @param tamanho Ver propriedade 'font' do HTML Canvas: https://www.w3schools.com/jsref/canvas_font.asp
 * @param cor Ver propriedade 'fillStyle' do HTML Canvas: https://www.w3schools.com/jsref/canvas_fillstyle.asp
 * @returns
 */
export function texto(texto: string, fonte: string = 'Arial', tamanho: string = '30px', cor: string = 'black') {
  return new TextoImagem(texto, fonte, tamanho, cor)
}
